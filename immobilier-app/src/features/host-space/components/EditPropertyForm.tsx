...
export const EditPropertyForm = ({ property }: { property: any }) => {
  const [currentStep, setCurrentStep] = useState(0);
  const [images, setImages] = useState<File[]>([]);
  const [previews, setPreviews] = useState<string[]>(property.media?.map((m: any) => m.url) || []);
  const [selectedFeatures, setSelectedFeatures] = useState<number[]>(property.features?.map((f: any) => f.id) || []);
  ...
  const {
    ...
    defaultValues: {
      ...
      category: property.category?.value || property.category || 'apartment',
      etat: property.etat?.value || property.etat || 'new',
      typeTransaction: property.typeTransaction?.value || property.typeTransaction || 'sale',
      address: property.address?.address || property.address || '',
      nbRooms: property.nbRooms || 1,
      nbBathrooms: property.nbBathrooms || 1,
      nbEtages: property.nbEtages || 1,
      etage: property.etage || 0,
      city: property.address?.city?.id || property.city || 1,
      latitude: property.location?.latitude || 30.4278,
      longitude: property.location?.longitude || -9.5981,
      features: selectedFeatures,
      dateConstruction: property.dateConstruction ? property.dateConstruction.split('T')[0] : ''
    }
  });
  ...
  const [trashImages, setTrashImages] = useState<number[]>([]);
  ...
  const removeImage = (index: number) => {
    // Determine if it was an existing property image or a newly uploaded one
    const existingImages = property.media || [];
    const existingImagesCount = existingImages.length;
    
    if (index < existingImagesCount) {
        // It's an existing image, track its ID for deletion
        const imageToDelete = existingImages[index];
        if (imageToDelete && imageToDelete.id) {
            setTrashImages(prev => [...prev, imageToDelete.id]);
        }
        // Remove from the property media array temporarily for display
        property.media = property.media.filter((_, i) => i !== index);
    } else {
        // It's a newly uploaded file
        const newImageIndex = index - existingImagesCount;
        const updatedFiles = images.filter((_, i) => i !== newImageIndex);
        setImages(updatedFiles);
        setValue('images', updatedFiles);
    }
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      success('Bien mis à jour !', 'Les modifications ont été enregistrées.');
      navigate('/host-space');
    } catch (err: any) {
      console.error('Submit error details:', err?.response?.data || err);
      const responseData = err?.response?.data;
      let errorMessage = 'Impossible de mettre à jour la propriété. Veuillez réessayer.';
                ...
                >
                  <div className="flex items-center gap-4 mb-2">
                    ...
                    <div>
                      ...
                      <p className="text-gray-500 font-medium">Modifiez les détails essentiels</p>
                    </div>
                  </div>
                ...
                >
                  ...
                  >
                    ...
                    <div className="relative z-10">
                      ...
                      <p className="text-2xl font-black text-gray-900">Ajouter des photos</p>
                      <p className="text-sm text-gray-400 font-medium mt-2">Les nouvelles photos s'ajouteront à votre annonce</p>
                    </div>
                  </div>
