    ...
    return (
      <div className="space-y-4">
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-gray-900">Avis des hôtes</h2>
              <p className="text-gray-600 text-sm">Ce que disent les hôtes</p>
            </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-8">
          <div className="text-center">
            ...
            <p className="text-gray-600 text-sm">
              Les avis des hôtes apparaîtront après les premiers séjours.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {reviews.map((review) => (
          <div key={review.id} className="bg-white border border-gray-100 rounded-xl p-5 hover:border-gray-200 transition-colors">
            <div className="flex gap-4">
              <Link to={`/host/profile/${review.host.id}`} className="flex-shrink-0">
                <Avatar className="h-12 w-12 hover:ring-2 hover:ring-emerald-200 transition-all cursor-pointer">
                  <AvatarImage 
                    src={review.host.profile} 
                    alt={`${review.host.firstName} ${review.host.lastName}`}
                    className="object-cover"
                  />
                  <AvatarFallback className="bg-gradient-to-br from-emerald-400 to-teal-500 text-white font-medium text-sm">
                    {getInitials(review.host.firstName, review.host.lastName)}
                  </AvatarFallback>
                </Avatar>
              </Link>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between mb-3">
                  <div className="min-w-0">
                    <Link to={`/host/profile/${review.host.id}`} className="block">
                      <h4 className="font-semibold text-gray-900 text-sm truncate hover:text-emerald-600 transition-colors cursor-pointer">
                        {review.host.firstName} {review.host.lastName}
                      </h4>
                    </Link>
                    <div className="inline-flex items-center gap-1 bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full text-xs font-medium mt-1">
                      <Shield size={10} />
                      Hôte
                    </div>
                  </div>
                ...
                <div className="border-t border-gray-100 pt-3">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs">
                    {[
                      ...
                      { label: "Respect des règles", value: review.observanceHouseRules },
